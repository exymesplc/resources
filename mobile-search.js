(function(){
// 1. STUB & LIBRARIES
var libPath = "https://cdn.jsdelivr.net/npm/search-insights@2.11.0/dist/search-insights.min.js";
...
function show(q,hits){
    ...
    if(!hits.length){list.innerHTML='<div style="padding:40px;text-align:center;color:#5D6376;">No results</div>';}
        ...
        el.onclick=function(){
            if(window.aa) {
                window.aa('clickedObjectIDs', { index: el.getAttribute('data-algolia-index'), eventName: 'Result Click', objectIDs: [h.objectID] });
            }
            window.open(u,"_blank","noopener");p.remove();
        };
