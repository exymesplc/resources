...
window.srch=function(q){
    loadAlgolia(function(){
        ...
        Promise.all([
            c.initIndex("www_exymesplc_com_e3wswa1rj6_pages").search(q,{hitsPerPage:6}),
            c.initIndex("exymesplc_pdfs").search(q,{hitsPerPage:4})
        ]).then(function(r){
...
function show(q,hits){
    ...
    // Responsive width: 90vw for mobile, 60vw for tablets
    var w = window.innerWidth < 600 ? "90vw" : "60vw";
    p.style.cssText="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:"+w+";max-height:75vh;background:#fff;border-radius:12px;box-shadow:0 16px 64px rgba(14,55,71,0.3);z-index:999999;display:flex;flex-direction:column;overflow:hidden;font-family:Geist,sans-serif;";
...
function inject(){
    // ONLY INJECT IF SCREEN IS MOBILE OR TABLET (< 1024px)
    if (window.innerWidth >= 1024 || document.getElementById("mob-search-wrap")) return;
...
// Check every 200ms for the Nav Column, but only if we are on mobile/tablet
var poll=setInterval(function(){
    if(window.innerWidth < 1024 && document.querySelectorAll('[data-framer-name="Nav Column"]').length){
        inject();
        clearInterval(poll);
    }
},200);
